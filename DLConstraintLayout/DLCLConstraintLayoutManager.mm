...
#include "DLCLConstraintLayoutSolverCALayerTrait.hh"

typedef dlcl::solver<CALayer> solver_type;
...
@implementation DLCLConstraintLayoutSolver {
	std::unique_ptr<dlcl::solver<CALayer>> solver;
}

- (id)initWithLayer:(CALayer *)layer {
	NSAssert(layer, @"Method argument 'layer' must not be nil.");
	if ((self = [self init])) {
		self->solver = solver_ptr(new solver_type(layer));
	}
	return self;
}
...
#define DLCL_BENCHMARK 0
	...
	NSLog(@"Took %.8f seconds to run %s %d times (@ %.8f seconds each, on average).", interval, __PRETTY_FUNCTION__, iterations, interval / iterations);
#else
	[[self solverForLayer:layer] solveLayout];
#endif
}
